...
import {DatePipe, DecimalPipe, NgClass, NgForOf, NgIf, registerLocaleData} from "@angular/common";
...
import {NgxStarsControlComponent} from "./NgxStarsControlComponent";
...
export class ClientUpdateComponent implements OnInit{
    ...
    ngOnInit(): void {
        ...
        // this.getBesoinsTitles();
        // console.warn("clienttttttttttttttttttt",this.client)


    }
    // getBesoinsTitles() {
    //     if (this.client.besoins && Array.isArray(this.client.besoins) && this.client.besoins.length > 0) {
    //         this.besoinsTitles = this.client.besoins.map((besoin: any) => ({
    //             label: besoin.titre,
    //             value: besoin,
    //             id: besoin.id // Pour accéder à l'identifiant
    //         }));
    //         console.log("Besoins Titles:", this.besoinsTitles);
    //     } else {
    //         console.warn("Aucun besoin trouvé pour ce client.");
    //         this.besoinsTitles = []; // Remettez à zéro si aucun besoin
    //     }
    // }
    ...
    loadClientData(id: number) {
        this.clientService.getClientByid(id).subscribe({
            next: (clientData) => {
                ...
                // Charger directement les besoinsTitles
                this.besoinsTitles = this.client.besoins.map((besoin: any) => ({
                    label: besoin.titre,   // Texte à afficher dans le dropdown
                    value: besoin,         // Valeur associée
                    id: besoin.id          // ID pour référence
                }));

                if (!Array.isArray(this.client.actions)) {
                    this.client.actions = [];
                }
...
/////////////////////////////// Beoins technologies //////////////////////////////////
    ...
    /**
     * Fonction pour réinitialiser les champs du formulaire avec des valeurs par défaut.
     */
    resetActionsForm(): void {
        this.action = {
            id: null,
            dateMiseAJour: '',
            dateDeCreation: '',
    ...
    onSubmitAction() {
        ...
        this.actionService.addActionToClient(this.getDecryptedId(), this.action)
            .pipe(
                finalize(() => {
                    this.isLoading = false; // Cache le spinner
                    this.progress = 100; // Terminer à 100 %
    ...
    onDialogClose(dropdown: any) {
        this.displayBesoinDetailsDialog = false; // Met à jour l'état de la boîte de dialogue
        setTimeout(() => {
            dropdown.overlayVisible = true; // Rouvre le menu déroulant après la fermeture
        }, 0); // Reprogramme l'ouverture après la fermeture du dialogue
    }
    ...
    deleteAction(actionId: number): void {
        this.actionService.deleteAction(actionId).subscribe({
            next: (response) => {
                ...
                // Update UI to reflect deletion
                this.client.actions = this.client.actions.filter((action: any) => action.id !== actionId);
